...
export default function UserDashboard() {
  ...
  const fetchData = async () => {
    ...
    const [resBooks, resDash] = await Promise.all([
      fetch('https://vgn-library-system-1.onrender.com/api/books', { headers }),
      fetch('https://vgn-library-system-1.onrender.com/api/users/dashboard', { headers })
    ]);
  ...
  const handleBorrow = async (bookId: number) => {
    const res = await fetch('https://vgn-library-system-1.onrender.com/api/books/borrow', {
      method: 'POST',
