...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch('https://vgn-library-system.onrender.com/api/auth/register', {
      method: 'POST',
    ...
    <div className="max-w-md mx-auto mt-12 bg-white p-8 rounded-xl shadow border border-slate-100">
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-lg">
          <button type="button" onClick={() => setForm({...form, role: 'user'})} className={`py-2 text-xs font-bold rounded-md transition ${form.role === 'user' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>🧑‍🎓 Student / User</button>
